...
export default function Logo({ containerClassName = "", logoClassName = "", variant = 1 }: { containerClassName?: string; logoClassName?: string; variant: number }) {
  ...
  }, [variant, imageLoaded, setIsComponentLoading]);
  ...
  return (
    <section id="logo" className={containerClassName}>
      {(variant === 1 || variant === 2) && (
        <img
          src={logoVariant()}
          alt="Grand Theft Auto VI Logo"
          draggable={false}
          onContextMenu={(e) => e.preventDefault()} // Prevents users from saving image
          className={`${logoClassName} pointer-events-none`}
          onLoad={handleImageLoad}
        />
      )}
      {variant === 3 && (
        <div
          className={`${logoClassName} text-white select-none`}
          aria-hidden="true"
          onDragStart={(e) => e.preventDefault()} // Prevents users from dragging svg image
          onContextMenu={(e) => e.preventDefault()} // Prevents users from saving image
        >
          <svg fill="none" height="100%" width="100%" viewBox="0 0 46 35" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false" className="pointer-events-none">
            <path d="M46.2661 0.800781H34.7236V34.4008H46.2661V0.800781Z" fill="currentColor"></path>
            <path d="M33.9162 0.800781L17.3358 34.4008L0.776855 0.800781H12.3139L17.3332 11.5445L22.3766 0.800781H33.9162Z" fill="currentColor"></path>
            <path
